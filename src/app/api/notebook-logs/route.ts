...
export async function POST(req: Request) {
  ...
  if (!moduleRecord || !moduleRecord.hasNotebookWizard) {
    return NextResponse.json({ error: "Модуль не поддерживает создание этого задания" }, { status: 400 });
  }

  const spec = NOTEBOOK_MODULE_SPECS[moduleRecord.order];
  if (!spec) {
    return NextResponse.json({ error: "Для этого модуля не заданы промпты" }, { status: 400 });
  }

  const existing = await prisma.card.findUnique({
    where: {
      userId_moduleId_kind: { userId: session.user.id, moduleId: moduleRecord.id, kind: "notebook_log" },
    },
  });
  if (existing) {
  ...
  const initial: NotebookCardJson = {
    prompts: spec.prompts.map((p) => ({ label: p.label, promptText: p.template, result: "" })),
  };
